...
class ScoreSdeVeScheduler(SchedulerMixin, ConfigMixin):
    ...
    def add_noise(
        ...
        sigmas = self.discrete_sigmas.to(original_samples.device)
        # print(f"sde add noise, sigmas length: {len(self.discrete_sigmas)}, {timesteps}")
    ...
    def add_noise_villandiff(
        ...
        sigmas = sigmas[timesteps]
        noise = (
            (noise + trigger) * sigmas[:, None, None, None]
            if noise is not None
            else torch.randn_like(original_samples) * sigmas[:, None, None, None]
        )
        noisy_samples = noise + original_samples
