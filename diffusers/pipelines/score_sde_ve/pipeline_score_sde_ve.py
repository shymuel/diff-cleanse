    ...
    def __call__(
        ...
        **kwargs,
        ...
        if init == None:
            ...
            sample = init.detach().clone() * self.scheduler.init_noise_sigma
            sample = sample.to(self.device)

        self.scheduler.set_timesteps(num_inference_steps)
        self.scheduler.set_sigmas(num_inference_steps)

        # mov = [sample.clamp(0, 1).cpu().permute(0, 2, 3, 1).numpy()]  # 26.9, 17.15, 17.15
        mov = []
        ...
        self.scheduler.set_timesteps(2000)
        self.scheduler.set_sigmas(2000)

        if not return_dict:
            return (sample, mov, [])

        return ImagePipelineOutput(images=sample, movie=mov, activations=[])
