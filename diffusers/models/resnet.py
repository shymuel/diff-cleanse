...
class ResnetBlock2D(nn.Module):
        ...
        *,
        ...
        if self.up:
            if kernel == "fir":
                ...
                self.upsample = self.upsample_fir
            elif kernel == "sde_vp":
            ...
            if kernel == "fir":
                fir_kernel = (1, 3, 3, 1)
                self.downsample = self.downsample_fir
    ...
    def upsample_fir(self, x):
        fir_kernel = (1, 3, 3, 1)
        return upsample_2d(x, kernel=fir_kernel)

    def downsample_fir(self, x):
        fir_kernel = (1, 3, 3, 1)
        return downsample_2d(x, kernel=fir_kernel)
