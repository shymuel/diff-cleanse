...
import diffusers
from diffusers import DDPMPipeline, DDPMScheduler, UNet2DModel, DDIMPipeline, DDIMScheduler, LDMPipeline
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_id", type=int, default=None)
    ...
    parser.add_argument("--lr_warmup_steps", type=int, default=0,
                        help="Number of steps for the warmup in the lr scheduler.")  # 500太多，改成50
    ...
    parser.add_argument("--loss_p", action="store_true")
...
def main(args):
    print("pruned model ckpt: ", args.pruned_model_ckpt)
    if "/" in args.model_path:
        model_name = args.model_path.split("/")[-1]
    elif "\\" in args.model_path:
        model_name = args.model_path.split("\\")[-1]

    if "baddiff" in args.pruned_model_ckpt:
        if "cifar10" in args.dataset:
            args.output_dir = os.path.join("G:/diff-cleanse_rm/finetune/baddiff_ddpm1_cifar10", model_name)
        else:
            args.output_dir = os.path.join("G:/diff-cleanse_rm/finetune/baddiff_ddpm3_celebahq256", model_name)
    elif "villandiff" in args.pruned_model_ckpt:
        if "cifar10" in args.dataset:
            if "DDPM" in args.model_path:
                args.output_dir = os.path.join("G:/diff-cleanse_rm/finetune/villandiff_ddpm1_cifar10", model_name)
        else:
            if "DDPM" in args.model_path:
                args.output_dir = os.path.join("G:/diff-cleanse_rm/finetune/villandiff_ddpm3_celebahq", model_name)

    logging_dir = os.path.join(args.output_dir, args.logging_dir)
    accelerator_project_config = ProjectConfiguration()
    accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps,
        mixed_precision=args.mixed_precision, log_with=args.logger,
    ...
    if os.path.isdir(args.model_path) and args.loss_p:
        print("Loading backdoored model from {}".format(args.model_path))
        pipeline_backdoored = DDPMPipeline.from_pretrained(args.model_path)
        model_f = pipeline_backdoored.unet.eval()
        del pipeline_backdoored

    model = pipeline.unet
    noise_scheduler = pipeline.scheduler

    # Get the datasets: you can either provide your own training and evaluation files (see below)
    if args.dataset == 'cifar10' or args.dataset == 'cifar10-syn':
        # 10% real data
        # dataset = utils.get_dataset("cifar10")
        # np.random.seed(0)
        # from torch.utils.data import Subset
        # print("len dataset: ", len(dataset))
        # indices = np.random.choice(range(len(dataset)), int(0.1*len(dataset)), replace=False)
        # dataset = Subset(dataset, indices)

        # cifar10_syn
        dataset = utils.get_dataset("cifar10-syn")
        print(args.dataset)
        ...
        # num_epochs = 100000 // len(train_dataloader) + 1
        # save_step_interval = 10000
    elif args.dataset == "celeba-hq" or args.dataset == "celeba-hq-syn":
        # dataset = utils.get_dataset("D:/celebahq256_img")
        dataset = utils.get_dataset(args.dataset)
        print(f"len dataset: {len(dataset)}")
        np.random.seed(0)
        from torch.utils.data import Subset
        indices = np.random.choice(range(len(dataset)), int(0.375*len(dataset)), replace=False)
        logger.info(f"len indices: {len(indices)}")
        ...
        num_epochs = 40
        save_epoch_interval = 5
    elif args.dataset == "celeba-hq-latent" or args.dataset == "celeba-hq-latent-syn":
        dataset = utils.get_dataset(args.dataset)
        print(f"len dataset: {len(dataset)}")
        np.random.seed(0)
        from torch.utils.data import Subset
    ...
    if args.use_ema:
        ema_model = EMAModel(model.parameters(), decay=args.ema_max_decay,
                             use_ema_warmup=False, inv_gamma=args.ema_inv_gamma,
                             power=args.ema_power, model_cls=UNet2DModel, model_config=model.config)

    # Initialize the optimizer
    if "CELEBA" in args.model_path:
        args.learning_rate = 2e-5  # finetune 6e-5, scratch 2e-5
    else:
    ...
    if args.loss_p:
        model_f = model_f.to(accelerator.device)
    ...
    if accelerator.is_main_process:
        ...
        pipeline = DDIMPipeline(unet=unet,
            scheduler=DDIMScheduler(num_train_timesteps=args.ddpm_num_steps))
        pipeline.scheduler.set_timesteps(num_inference_steps=args.ddim_num_inference_steps, start_inference_step=0)
        images = pipeline(batch_size=args.eval_batch_size,
    ...
    for epoch in range(first_epoch, num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.resume_from_checkpoint and epoch == first_epoch and step < args.resume_step:
            ...
            with accelerator.accumulate(model):
                ...
                if args.loss_p:
                    model_output_f = model_f(noisy_images, timesteps).sample
                    loss += (model_output_f - model_output).square().sum(dim=(1, 2, 3)).mean(dim=0)
        ...
        if epoch == 0 or (epoch+1) % save_epoch_interval == 0:  # Save the model & generate sample images every 5 epoch
            # if num_step == 1 or \
            #         num_step % save_step_interval == 0:  # Save the model & generate sample images every 5 epoch
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    # save the model
                    unet = accelerator.unwrap_model(model).eval()
                    ...
                    if args.use_ema:
                        ema_model.store(unet.parameters())
                        ema_model.copy_to(unet.parameters())
                        # if epoch == 14 or epoch == num_epochs:
                        torch.save(unet, os.path.join(args.output_dir, 'pruned', f'unet_ema_pruned_{epoch}.pth'))
                    else:
                        # if epoch == 14 or epoch == num_epochs:
                        torch.save(unet, os.path.join(args.output_dir, 'pruned', f'unet_pruned_{epoch}.pth'))
                    pipeline = DDPMPipeline(unet=unet, scheduler=noise_scheduler)
                    pipeline.save_pretrained(args.output_dir)

                    # generate images
                    logger.info("Sampling images...")
                    # if "baddiff" in args.output_dir:
                    #     pipeline = DDPMPipeline(unet=unet,
                    #                             scheduler=DDPMScheduler(num_train_timesteps=args.ddpm_num_steps))
                    # elif "villandiff" in args.output_dir:

                    pipeline = DDIMPipeline(unet=unet, scheduler=DDIMScheduler(num_train_timesteps=args.ddpm_num_steps))
                    pipeline.scheduler.set_timesteps(args.ddim_num_inference_steps)
                    images = pipeline(batch_size=args.eval_batch_size, num_inference_steps=args.ddim_num_inference_steps,
                        output_type="numpy").images

                    if args.use_ema:
                        ema_model.restore(unet.parameters())
                    torchvision.utils.save_image(torch.from_numpy(images).permute([0, 3, 1, 2]),
                                                 os.path.join(args.output_dir, 'vis', f'epoch_{epoch}.png'))
