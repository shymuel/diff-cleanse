...
def ddpm_steps_bd(x, seq, model, b, miu, coef_miu, args, **kwargs):
    with torch.no_grad():
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), total=len(seq)):
            ...
            miu_ = torch.stack([miu.to(device)] * batch)  # (batch,3,32,32)
            # 用噪声计算x0的方式就是论文里formula (9)下面那个
            x0_from_e = (1.0 / at).sqrt() * x - (1.0 / at - 1).sqrt() * e * args.gamma - (1.0 / at - 1).sqrt() * miu_

            if args.trigger_type == 'patch':  # patch的算法完全不一样，相当于miu*1
                tmp_x0 = (1.0 / at).sqrt() * x - (1.0 / at - 1).sqrt() * e
                tmp_x0[:, :, -args.patch_size:, -args.patch_size:] = x0_from_e[:, :, -args.patch_size:,
                                                                     -args.patch_size:]
                x0_from_e = tmp_x0
            ...
            # 求均值，求方差
            mean_eps = ((atm1.sqrt() * beta_t) * x0_from_e + ((1 - beta_t).sqrt() * (1 - atm1)) * x + coef_miu[
                i] * miu_) / (1.0 - at)
            mean = mean_eps
            ...
            if args.trigger_type == 'patch':
                ...
                tmp_sample[:, :, -args.patch_size:, -args.patch_size:] = sample[:, :, -args.patch_size:,
                                                                         -args.patch_size:]
                sample = tmp_sample

            xs.append(sample.to('cpu'))

    return xs, x0_preds, e