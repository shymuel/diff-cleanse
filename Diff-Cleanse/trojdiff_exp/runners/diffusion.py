...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        self.build_model()
        logging_dir = os.path.join(args.output_path, args.logging_dir)
    ...
    def build_model(self):
        ...
        if args.pruned_model is not None:
            print("Loading pruned model from {}".format(args.pruned_model))
            states = torch.load(args.pruned_model, map_location='cpu')
            ...
            #     else:
            #         print(states[i])
            # print(len(states), type(states[0]))  # 5, <class 'collections.OrderedDict'>
            # states = states[-1]
        ...
        elif not self.args.use_pretrained:  # 好像并没有被用到
    ...
    def train(self, loss_p=False):
        ...
        if loss_p:
            ...
            print("Loading teacher model from {}".format(args.model_path))
        ...
        if args.dataset == "cifar10" or args.dataset == "celeba":
            ...
            if config.data.random_flip is False:
                ...
                tran_transform = transforms.Compose(
                    ...
                    transforms.ToTensor(),
                    transforms.Lambda(lambda x: normalize(vmin_in=0, vmax_in=1, vmin_out=-1.0, vmax_out=1.0, x=x)),])

            from utils import get_dataset_util
            dataset = get_dataset_util("cifar10-syn", transform=tran_transform)
            print(f"dataset size: {len(dataset)}")
        elif args.dataset == "celeba-syn":
            transform = transforms.Compose(
                [transforms.Resize(config.data.image_size),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor()])
            dataset = datasets.ImageFolder(
                root='D:/00_dataset/celeba_syn',
                transform=transform
            )
        ...
        if accelerator.is_main_process:
            ...
            with torch.no_grad():
                ...
                os.makedirs(os.path.join(self.args.log_path, "vis"), exist_ok=True)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if step % 10 == 0:
                    logging.info(f"step: {step} (Ep={epoch}/{self.config.training.n_epochs}, Iter={i}/{len(train_loader)}), "
