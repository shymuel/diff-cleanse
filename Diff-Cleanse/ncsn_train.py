...
import diffusers
from diffusers import UNet2DModel, ScoreSdeVePipeline, ScoreSdeVeScheduler
...
def parse_args():
    ...
    parser.add_argument("--ncsn_num_steps", type=int, default=1000, help="train steps of NCSN models")
    parser.add_argument("--ncsn_num_inference_steps", type=int, default=1000)
...
def main(args):
    ...
    args.output_dir = os.path.join("G:/diff-cleanse_rm/finetune/villandiff_ncsn_cifar10", model_name)
    ...
    if args.pruned_model_ckpt is not None:
        ...
        pipeline = ScoreSdeVePipeline(unet=unet,
    ...
    if os.path.isdir(args.model_path) and args.loss_p:
        ...
        pipeline_backdoored = ScoreSdeVePipeline.from_pretrained(args.model_path)
    ...
    if accelerator.is_main_process:
        ...
        pipeline = ScoreSdeVePipeline(unet=unet,
            scheduler=ScoreSdeVeScheduler(num_train_timesteps=args.ncsn_num_steps))
        pipeline.scheduler.set_timesteps(num_inference_steps=args.ncsn_num_inference_steps)
        images = pipeline(batch_size=args.eval_batch_size,
            num_inference_steps=args.ncsn_num_inference_steps,
    ...
    for epoch in range(first_epoch, num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            timesteps = torch.randint(
                low=0, high=noise_scheduler.config.num_train_timesteps, size=(bsz,),
            ...
            with accelerator.accumulate(model):
                ...
                # print("sigmas_t shape: ", sigmas_t.shape)
                model_output = model(noisy_images, sigmas_t).sample
                loss = (noise + model_output * unqueeze_n(noisy_images, sigmas_t)).square().sum(dim=(1, 2, 3)).mean(dim=0)
                if args.loss_p:
                    model_output_f = model_f(noisy_images, sigmas_t).sample
                    loss += lambda_ * (model_output_f - model_output).square().sum(dim=(1, 2, 3)).mean(dim=0)

                accelerator.backward(loss)
                if accelerator.sync_gradients: 
                    accelerator.clip_grad_norm_(model.parameters(), 1.0)
                optimizer.step()
                lr_scheduler.step()  # corresponding explanations are at https://huggingface.co/docs/accelerate/v0.29.2/en/usage_guides/gradient_accumulation#the-finished-code
        ...
        if epoch == 0 or (epoch+1) % save_epoch_interval == 0:  # Save the model & generate sample images every 5 epoch
            ...
            #         num_step % save_step_interval == 0:  # Save the model & generate sample images every 5 epoch
                ...
                if accelerator.is_main_process:
                    ...
                    pipeline = ScoreSdeVePipeline(unet=unet, scheduler=noise_scheduler)
                    pipeline.save_pretrained(args.output_dir)

                    # generate images
                    logger.info("Sampling images...")
                    pipeline = ScoreSdeVePipeline(unet=unet, scheduler=ScoreSdeVeScheduler(num_train_timesteps=args.ncsn_num_steps))
                    pipeline.scheduler.set_timesteps(args.ncsn_num_inference_steps)
                    images = pipeline(batch_size=args.eval_batch_size, num_inference_steps=args.ncsn_num_inference_steps,
                        output_type="numpy").images
