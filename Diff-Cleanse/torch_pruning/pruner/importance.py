...
class CLPImportance(Importance):
    """
                    for name, m in pipeline.unet.named_modules():
                    if isinstance(m, nn.Conv2d):
                        channel_lips = []
                        for idx in range(m.weight.shape[0]):
                            w = m.weight[idx].reshape(m.weight.shape[1], -1).abs()
                            channel_lips.append(torch.svd(w.cpu())[1].max())  # 计算该输出通道的最大奇异值
