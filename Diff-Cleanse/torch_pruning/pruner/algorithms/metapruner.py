...
class MetaPruner:
    ...
    def __init__(
        ...
        pruning_ratio_start: float = 0,  # 我后加的，算一个剪枝的index的起点吧
        ...
        # diffusion cleanse
        init_importance: torch.Tensor = None,
    ):
        self.model = model
        self.importance = importance

        self.init_importance = init_importance  # diffusion cleanse
        ...
        self.pruning_ratio_start = pruning_ratio_start
    ...
    def manual_prune(self, layer, pruning_fn, pruning_ratios_or_idxs):
        ...
        if isinstance(pruning_ratios_or_idxs, float):
            ...
            if self.init_importance != None:
                imp = - self.estimate_importance(full_group) - self.init_importance
            else:
                imp = self.estimate_importance(full_group)
            self.importance_results = imp
            imp_argsort = torch.argsort(imp)  # 这里排序
    ...
    def prune_local(self) -> typing.Generator:
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types)):
            ...
            if self._check_pruning_ratio(group): # check pruning ratio
                ...
                # print(f"i: {i}, ch_groups: {ch_groups}")
                # print(group)
                if self.init_importance != None and i < len(self.init_importance) and self.init_importance[i] != None:
                    # print(f"shape: {self.init_importance[i].shape}, {self.estimate_importance(group).shape}")
                    imp = self.estimate_importance(group) + self.init_importance[i]  # backdoor - clean
                    # imp = - self.estimate_importance(group) - self.init_importance[i]  # clean - backdoor
                else:
                    imp = self.estimate_importance(group)
                self.importance_results.append(imp)
                ...
                if self.DG.is_out_channel_pruning_fn(pruning_fn):
                    ...
                    n_pruned_start = current_channels - int(self.layer_init_out_ch[module] * (1 - self.pruning_ratio_start))
                    n_pruned = current_channels - int(self.layer_init_out_ch[module] * (1 - target_pruning_ratio))
                else:
                    current_channels = self.DG.get_in_channels(module)
                    target_pruning_ratio = self.get_target_pruning_ratio(module)
                    n_pruned_start = current_channels - int(self.layer_init_in_ch[module] * (1 - self.pruning_ratio_start))
                    n_pruned = current_channels - int(self.layer_init_in_ch[module] * (1 - target_pruning_ratio))
                # round to the nearest multiple of round_to
                if self.round_to:
                    print("round")
                    n_pruned_start = self._round_to(n_pruned_start, current_channels, self.round_to)
                ...
                if n_pruned > 0:
                    if (self.prune_head_dims and _is_attn) or (not _is_attn):
                        n_pruned_start_per_group = n_pruned_start // ch_groups
                        n_pruned_per_group = n_pruned // ch_groups
                        if self.round_to:
                            print("round")
                            n_pruned_start_per_group = self._round_to(n_pruned_start_per_group, group_size, self.round_to)
                        ...
                        if n_pruned_per_group > 0:
                            for chg in range(ch_groups):
                                ...
                                sub_pruning_idxs = sub_imp_argsort[n_pruned_start_per_group:n_pruned_per_group] + chg*group_size # offset
                                pruning_idxs.append(sub_pruning_idxs)
                    # print(f"n_round>0, n start: {n_pruned_start}, n pruned: {n_pruned}, ch_groups: {ch_groups}")
                    # print(f"n start group: {n_pruned_start_per_group}, n pruned group: {n_pruned_per_group}")
                else:  # no channel grouping
                    imp_argsort = torch.argsort(imp)
                    pruning_idxs.append(imp_argsort[n_pruned_start:n_pruned] )
                    # print(f"n start: {n_pruned_start}, n pruned: {n_pruned}")
                ...
                if _is_attn and self.prune_num_heads: # Prune entire attn heads
                    ...
                    n_heads_removed_start = self.num_heads[qkv_layers[0]] - int(self.init_num_heads[qkv_layers[0]] * (1 - self.pruning_ratio_start))
                    ...
                    if n_heads_removed>0:
                        ...
                        for head_id in torch.argsort(head_imp)[n_heads_removed_start:n_heads_removed]:
                            pruning_idxs.append(torch.arange(head_id*group_size, (head_id+1)*group_size, device=head_imp.device) )

                        # print(f"head start: {n_heads_removed_start}, head pruned: {n_heads_removed}")
                ...
                if self.DG.check_pruning_group(group):
                    ...
                    if _is_attn and self.prune_num_heads and (n_heads_removed-n_heads_removed_start)>0:
                        for dep, _ in group:
                            if dep.target.module in self.num_heads:
                                self.num_heads[dep.target.module] -= (n_heads_removed-n_heads_removed_start)
                    yield group
    ...
    def prune_global(self) -> typing.Generator:
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types)):
            if self._check_pruning_ratio(group):
                ...
                self.init_importance.append(imp)
