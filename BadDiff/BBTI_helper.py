...
def get_model_optim_sched(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader):
    ...
    print(type(model), type(vae), type(optimizer), type(lr_sched))
...
def trigger_inversion(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, vae, get_pipeline,
    ...
    elif "DDPM" and "CELEBA-HQ" in config.ckpt:
        init_pattern = torch.randn((3, 256, 256), dtype=torch.float, device=device, requires_grad=True)
        # init_pattern = copy.deepcopy(target)
        # init_pattern = init_pattern.to(device)
        # init_pattern.requires_grad_(True)
...
# ddpm1-cifar10
# python BBTI_helper.py --project default --mode reverse --ckpt D:\BackdoorDiff-DS\VillanDiff\DDPM1-CIFAR10\res_DDPM-CIFAR10-32_CIFAR10_ep100_ode_c1.0_p0.1_BOX_14-BOX-CORNER_psi1.0 --fclip o --gpu 0 --batch 16 --save_epoch 1 --sched UNIPC-SCHED -is 20
# ddpm-celebahq
# python BBTI_helper.py --project default --mode reverse --ckpt D:\BackdoorDiff-DS\VillanDiff\DDPM-CELEBAHQ256\res_DDPM-CELEBA-HQ-256_CELEBA-HQ_ep1500_ode_c1.0_p0.2_GLASSES-CAT_psi1.0 --sched UNIPC-SCHED --fclip o --gpu 0 --batch 16 --save_epoch 1 -is 20