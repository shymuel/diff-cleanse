...
class DatasetLoader(object):
    ...
    def __init__(self, name: str, label: int = None, root: str = None, channel: int = None, image_size: int = None,
        ...
        self.__set_img_shape(image_size=image_size)  # 设置是1通道还是3通道图片
    ...
    def set_poison(self, trigger_type: str, target_type: str, target_dx: int = -5, target_dy: int = -3,
                   clean_rate: float = 1.0, poison_rate: float = 0.2) -> 'DatasetLoader':
        if self.__root == None:
    ...
    def __flex_sz_dataset(self):
        ...
        self.__full_dataset: datasets.DatasetDict = self.__dataset.train_test_split(train_size=train_n,
                                                                                    test_size=test_n)
        self.__full_dataset[DatasetLoader.TRAIN] = self.__full_dataset[DatasetLoader.TRAIN].add_column(
            DatasetLoader.IS_CLEAN, [True] * train_n)
        self.__full_dataset[DatasetLoader.TEST] = self.__full_dataset[DatasetLoader.TEST].add_column(
            DatasetLoader.IS_CLEAN, [False] * test_n)
        ...
        self.__full_dataset = concatenate_datasets(
            [self.__full_dataset[DatasetLoader.TRAIN], self.__full_dataset[DatasetLoader.TEST]])
        self.__full_dataset = self.__full_dataset.with_transform(trans)
    ...
    def get_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        return DataLoader(datasets, batch_size=self.__batch_size, shuffle=self.__shuffle, pin_memory=True,
                          num_workers=0)
    ...
    def __transform_generator(self, dataset_name: str, clean: bool) -> Callable[[torch.Tensor], torch.Tensor]:
        ...
        def backdoor_transforms(examples) -> DatasetDict:
            ...
            repeat_times = (data_shape[0], *([1] * len(data_shape[1:])))  # (16, 1, 1, 1)
            ...
            examples[DatasetLoader.TARGET] = self.__target.repeat(*repeat_times)  # target
            return examples
...
if __name__ == "__main__":
    ...
    dsl = DatasetLoader(root=ds_root, name=DatasetLoader.CIFAR10, batch_size=128).set_poison(
    ...
    # BOX_14 Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_STOP_SIGN_14, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax)
    backdoor.show_image(img=tr)
    # SM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_BOX_14, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax)
    backdoor.show_image(img=tr)
    # XSM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_BOX_11, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax)
    backdoor.show_image(img=tr)
    # XXSM_BOX Trigger
