...
DEFAULT_TRIGGER: str = Backdoor.TRIGGER_BOX_14  # grey box with size=14*14
...
DEFAULT_POSTFIX: str = ""
DEFAULT_FCLIP: str = 'o'
DEFAULT_SAVE_IMAGE_EPOCHS: int = 50
DEFAULT_SAVE_MODEL_EPOCHS: int = 50
...
def setup():
    ...
    config.gradient_accumulation_steps = int(bs // config.batch)  # enlarge the batchsize but save memory

    if args.mode == MODE_TRAIN or args.mode == MODE_TRAIN_MEASURE:
        setattr(config, "output_dir", os.path.join(config.result, naming_fn(config=config)))  # the long name folder
...
def get_accelerator(config: TrainingConfig):  # to accelerate the train process
    ...
    accelerator = Accelerator(
        ...
        # log_with=["tensorboard", "wandb"],  # the tools to record the log
...
def get_model_optim_sched(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader):
    if config.ckpt != None:
        if config.sample_ep != None and config.mode in [MODE_MEASURE, MODE_SAMPLING]:
            ...
            model, noise_sched = DiffuserModelSched.get_pretrained(ckpt=ep_model_path, noise_sched_type=config.sched, clip_sample=config.clip)
        # else:
        ...
        #     print(Log.warning(f"No such pretrained model: {ep_model_path}, load from ckpt: {config.ckpt}"))
            ...
            model, noise_sched, get_pipeline = DiffuserModelSched.get_pretrained(ckpt=config.ckpt,
                                                                                 noise_sched_type=config.sched,
                                                                                 clip_sample=config.clip)
        optimizer = torch.optim.Adam(model.parameters(), lr=config.learning_rate)
    else:
        model, noise_sched, get_pipeline = DiffuserModelSched.get_model_sched(image_size=dataset_loader.image_size,
...
def init_train(config: TrainingConfig, dataset_loader: DatasetLoader):
    ...
    model, optimizer, lr_sched, noise_sched, cur_epoch, cur_step, get_pipeline = get_model_optim_sched(config=config,
                                                                                                       accelerator=accelerator,
...
def sampling(config: TrainingConfig, file_name: Union[int, str], pipeline):
    def gen_samples(init: torch.Tensor, folder: Union[os.PathLike, str]):
        ...
        # print(t_index)
        # r = int(pow(config.eval_sample_n, 0.5))
        # c = int(pow(config.eval_sample_n, 0.5))
        # print('r, c: ', r, c)
        # clip_opt = "" if config.clip else "_noclip"
        # for t in t_index:
...
def measure(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader,
    ...
    def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], trigger: torch.Tensor = None,
                            num_inference_steps: int = 1000, ddim_eta: float = None, max_batch_n: int = 256,
    ...
    if config.task != TASK_GENERATE:
        ...
        if re_comp_backdoor_metric or recomp:
            ...
            gen_backdoor_target = ImagePathDataset(path=backdoor_path, njobs=4)[:].to(device)
...
def checkpoint(config: TrainingConfig, accelerator: Accelerator, pipeline, cur_epoch: int, cur_step: int, repo=None,
    ...
    print("data ckpt path: ", config.data_ckpt_path)
    ...
    pipeline.save_pretrained(config.output_dir)  # 要把
...
def train_loop(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, get_pipeline, noise_sched,
    ...
    try:
        ...
        for epoch in range(int(start_epoch), int(config.epoch)):
            ...
            for step, batch in enumerate(loader):  # loader给出的target和clean?
                # memlog.append()
                # clean_images = batch['images']
                # 对于干净样本，pixel_values，也就是R是0，target是自身
                # 对于后门样本，pixel_values是加了trigger的图片，target是target图片
...
if __name__ == '__main__':
    ...
    accelerator, repo, model, noise_sched, optimizer, dataloader, lr_sched, cur_epoch, cur_step, get_pipeline = init_train(
    ...
    if config.mode == MODE_TRAIN or config.mode == MODE_RESUME or config.mode == MODE_TRAIN_MEASURE:
        ...
        if config.sample_ep is not None:
            sampling(config=config, file_name=int(config.sample_ep), pipeline=pipeline)
        else:
            sampling(config=config, file_name="final", pipeline=pipeline)
    else:
