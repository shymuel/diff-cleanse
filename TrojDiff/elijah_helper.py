...
def parse_args_and_config():
    ...
    parser.add_argument('--ckpt_path', type=str, help='The path of model')
...
@torch.no_grad()
def sample_with_trigger(args, runner, trigger=None, use_trigger_input_options=(False, True), use_backdoor_sampling_options=(False, True), R_coef_T=None, recomp=False, save_res_dict=False):
    ...
    score_json_path = "elijah_trigger_inversion.json"
    score_dict = {}
    if os.path.isfile(score_json_path):
        with open(score_json_path, 'r') as file:
            score_dict = json.load(file)
    model_name = args.ckpt_path.split("\\")[-2]  # -2 for clean models, -3 for baackdoored models
    if model_name not in score_dict.keys():
        score_dict[model_name] = {}
    R_coef_T = str(R_coef_T)
    if R_coef_T not in score_dict[model_name].keys():
        score_dict[model_name][R_coef_T] = {}
    ...
    images_tensor = torch.load(generated_img_ptfile)
...
def invert_trigger(args, runner):
    R_coef_T = 0.5
    ...
    if not os.path.isfile(trigger_filename):
        ...
        if args.dataset == "celeba":
            trigger = torch.rand([1, 3, 64, 64]).cuda()
        else:
    ...
    sample_with_trigger(args, runner, trigger, use_trigger_input_options=(True, ), use_backdoor_sampling_options=(True, ), R_coef_T=R_coef_T, save_res_dict=True)
