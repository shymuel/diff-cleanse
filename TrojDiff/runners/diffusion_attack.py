...
class Diffusion(object):
    ...
    def sample(self):
        ...
        if self.args.ckpt_path:
            self.args.log_path = self.args.ckpt_path
    ...
    def sample_fid(self, model, clean_image_folder=""):
        config = self.config

        if clean_image_folder != "":
            image_folder = clean_image_folder
        else:
    ...
    def sample_fid_bd(self, model, bd_image_folder=""):
        config = self.config
        if bd_image_folder != "":
            image_folder = bd_image_folder
        else:
            if self.args.use_pretrained:
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                    range(n_rounds), desc="Generating image samples for backdoor metrics."
            ):
                n = config.sampling.batch_size
    ...
    def measure(self, num_clean, num_backdoor):
        ...
        if self.args.dataset == "cifar10":
            ...
            pipe = pipeline("image-classification",
                    model="G:\\classifiers\\aaraki-vit-base-patch16-224-in21k-finetuned-cifar10", device="cuda:0")
            for img_name in os.listdir(bd_image_folder):
                img_path = os.path.join(bd_image_folder, img_name)
                image = Image.open(img_path).convert("RGB")  
                predictions = pipe(image)
                # print(predictions)
                top_prediction = max(predictions, key=lambda x: x['score'])
                if top_prediction['label'] == 'horse': # 7 is the number of class "horse"
                    correct_predictions += 1
                total_images += 1
        elif self.args.dataset == "celeba":
            ckpt_path = "D:\\Diff-Cleanse-draft\\trojdiff\\celeba-classifier\\celeba_classifier_resnet18.pth"
            import torchvision.models as models
            model = models.resnet18(weights=None, num_classes=8).to(self.device)
            ...
            for img_name in os.listdir(bd_image_folder):
                ...
                if predicted == self.args.target_label:
                    correct_predictions += 1
                total_images += 1

        asr_sc = correct_predictions / total_images
        print(f"ASR: {asr_sc}")

        score_dict = {fid_key: fid_sc,asr_key: asr_sc}
