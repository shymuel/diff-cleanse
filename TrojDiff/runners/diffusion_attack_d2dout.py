...
class Diffusion(object):
    ...
    def measure(self, num_clean, num_backdoor):
        ...
        if self.args.ckpt_path:
            ...
            max_num = -1
            latest_file = None
            for file_name in os.listdir(model_dir):  # 遍历 model_dir 文件夹中的所有文件
                match = pattern.match(file_name)
                if match:
                    num = int(match.group(1))
                    if num > max_num:  # 更新最大 num 和相应的文件
                        max_num = num
        ...
        # use the classifier to calculate the asr score
        # for dout, we only use MNIST as the outside dataset
        from transformers import pipeline
        pipe = pipeline("image-classification", model="G:\\classifiers\\farleyknight-org-username-vit-base-mnist",
                        device="cuda:0") # load the mnist classifier
        ...
        for img_name in os.listdir(bd_image_folder):
            ...
            if predictions[0]['label'] == str(self.args.target_label):
