...
def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], num_inference_steps: int=1000, ddim_eta: float=None, init: torch.Tensor=None, max_batch_n: int=256, rng: torch.Generator=None):
    if init == None:
        if sample_n > max_batch_n:
            ...
            residual = sample_n % max_batch_n
            batch_sizes = [max_batch_n] * (replica) + ([residual] if residual > 0 else [])
            init = [torch.randn(max_batch_n, 3, 32, 32, device="cuda") for _ in range(replica)]
            init.append(torch.randn(residual, 3, 32, 32, device="cuda"))
        else:
            batch_sizes = [sample_n]
            init = [torch.randn(sample_n, 3, 32, 32, device="cuda")]
    else:
        init = torch.split(init, max_batch_n)
        batch_sizes = list(map(lambda x: len(x), init))
    sample_imgs_ls = []
...
class DiffuserModelSched():
    ...
    def get_pretrained(ckpt: str, clip_sample: bool=None, clip_sample_range: float=None, noise_sched_type: str=None, num_inference_steps: int=1000, sde_type: str=SDE_VP):
        if ckpt == DiffuserModelSched.DDPM_CIFAR10_32:
            ...
            ckpt: str = "D:/Diff-Cleanse-draft/models/ddpm-cifar10-32"
        elif ckpt == DiffuserModelSched.DDPM_CELEBA_HQ_256:
            # ckpt: str = "google/ddpm-ema-celebahq-256"
            ckpt: str = "D:/Diff-Cleanse-draft/models/ddpm-ema-celebahq-256"
            ...
            ckpt: str = "D:\\Diff-Cleanse-draft\\villandiff\\ldm-celebahq-256"
