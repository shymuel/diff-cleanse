...
def setup():
    ...
    if args.mode == MODE_REVERSE:
        ...
        args_path = os.path.join(model_path, args.ckpt) 
...
def trigger_inversion(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, get_pipeline,
                      noise_sched,
    ...
    pipeline = get_pipeline(accelerate=accelerator, unet=accelerator.unwrap_model(model), vae=None,
    ...
    if "CIFAR10" in config.ckpt:  # hyperparameters
        # init_pattern = torch.ones((3, 32, 32), dtype=torch.float, device=device, requires_grad=True)
        init_pattern = torch.randn((3, 32, 32), dtype=torch.float, device=device, requires_grad=True)

        
        print(init_pattern.shape)
        # init_pattern = torch.rand([3, 32, 32]).cuda() * 2 - 1
        # init_pattern = copy.deepcopy(trigger)
    ...
    # init_pattern = init_pattern * 0.25 
    # init_pattern = (init_pattern - 0.5) * 2 
    ...
    for epoch in range(100):  # the reverse process
        ...
        pipline_res = pipeline(batch_size=config.batch, num_inference_steps = 1000,
        ...
        if loss_s != 0 and value_loss != 0:
            ...
            pattern.grad = torch.mean(x_T.grad, dim=0, keepdim=False)  
            ...
            entropy_loss_g.backward(retain_graph=True)  
            optimizer.step()
...
if __name__ == '__main__':
    ...
    if config.mode == MODE_REVERSE:
        score_dict = {}
        pipeline = trigger_inversion(config, accelerator, repo, model, get_pipeline, noise_sched, optimizer, dataloader,
                                     lr_sched,
...
# python BBTI_helper_ncsn.py --project 1 --mode reverse --ckpt E:\res_NCSN_CIFAR10_my_CIFAR10_ep100_sde_c1.0_p0.5_SM_STOP_SIGN-FEDORA_HAT_psi0.0_lr2e-05_rhw1.0_rhb0.0_flex_new-set --sched SCORE-SDE-VE-SCHED --fclip o --gpu 0 --batch 16 --save_epoch 1 -is 1000
