...
DEFAULT_SOLVER_TYPE: str = 'sde'  # ncsn设置为sde
DEFAULT_PSI: float = 1
...
MODE_SAMPLING_OPTS = ['project', 'task', 'sched', 'ddim_eta', 'infer_steps', 'mode', 'eval_max_batch', 'gpu', 'fclip',
                      'ckpt', 'sample_ep', 'infer_start', 'inpaint_mul']
MODE_MEASURE_OPTS = ['project', 'task', 'sched', 'ddim_eta', 'infer_steps', 'mode', 'eval_max_batch', 'gpu', 'fclip',
                     'ckpt', 'sample_ep', 'infer_start', 'inpaint_mul', 'remove_backdoor']
...
def parse_args():
    ...
    parser.add_argument('--mode', '-m', required=True, type=str, help='Train or test the model',
                        choices=[MODE_TRAIN, MODE_RESUME, MODE_SAMPLING, MODE_MEASURE, MODE_TRAIN_MEASURE])
    parser.add_argument('--task', '-t', required=False, type=str, help='Type of task for performance measurement',
                        choices=[TASK_GENERATE, TASK_UNPOISONED_DENOISE, TASK_POISONED_DENOISE,
                                 TASK_UNPOISONED_INPAINT_BOX, TASK_POISONED_INPAINT_BOX, TASK_UNPOISONED_INPAINT_LINE,
                                 TASK_POISONED_INPAINT_LINE])
    parser.add_argument('--dataset', '-ds', type=str, help='Training dataset',
                        choices=[DatasetLoader.MNIST, DatasetLoader.CIFAR10, DatasetLoader.CELEBA,
                                 DatasetLoader.CELEBA_HQ, DatasetLoader.CELEBA_HQ_LATENT_PR05,
                                 DatasetLoader.CELEBA_HQ_LATENT, DatasetLoader.CIFAR10_SYN, DatasetLoader.CELEBA_HQ_SYN,
                                 DatasetLoader.CELEBA_HQ_LATENT_SYN])
    parser.add_argument('--sched', '-sc', type=str, help='Noise scheduler',
                        choices=["DDPM-SCHED", "DDIM-SCHED", "DPM_SOLVER_PP_O1-SCHED", "DPM_SOLVER_O1-SCHED",
                                 ...
                                 "LMSD-SCHED", "SCORE-SDE-VE-SCHED", "EDM-VE-SDE-SCHED", "EDM-VE-ODE-SCHED"])
    parser.add_argument('--ddim_eta', '-det', type=float,
                        help=f'Randomness hyperparameter \eta of DDIM, range: [0, 1], default: {DEFAULT_DDIM_ETA}')
    ...
    parser.add_argument('--ext_poison_rate', '-epr', type=float,
                        help=f"Poison rate for extend backdoor dataset, default for train: {DEFAULT_EXTEND_POISON_RATE}")
    ...
    parser.add_argument('--dataset_load_mode', '-dlm', type=str,
                        ...
                        choices=[DatasetLoader.MODE_FIXED, DatasetLoader.MODE_FLEX, DatasetLoader.MODE_EXTEND,
                                 DatasetLoader.MODE_NONE])
    parser.add_argument('--solver_type', '-solt', type=str,
                        help=f"Target solver type of backdoor training, default for train: {DEFAULT_SOLVER_TYPE}",
                        choices=['sde', 'ode'])
    parser.add_argument('--sde_type', '-sdet', type=str,
                        help=f"Diffusion model type, default for train: {DEFAULT_SDE_TYPE}",
                        choices=["SDE-VP", "SDE-VE", "SDE-LDM"])
    parser.add_argument('--psi', '-ps', type=float,
                        help=f"Backdoor scheduler type, value between [1, 0], default for train: {DEFAULT_PSI}")
    parser.add_argument('--ve_scale', '-ves', type=float,
                        help=f"Variance Explode correction term scaler, default for train: {DEFAULT_VE_SCALE}")
    parser.add_argument('--vp_scale', '-vps', type=float,
                        help=f"Variance Preserve correction term scaler, default for train: {DEFAULT_VP_SCALE}")
    ...
    parser.add_argument('--R_trigger_only', '-trigonly', action='store_true',
                        help="Making poisoned image without clean images")
...
def setup():
    ...
    # args.ckpt = format_ckpt_dir(args.ckpt)
    print("args.ckpt: ", args.ckpt, os.path.join(args.ckpt, args_file))
    ...
    bs = 0
    if config.dataset in [DatasetLoader.CIFAR10, DatasetLoader.CIFAR10_SYN, DatasetLoader.MNIST,
                          DatasetLoader.CELEBA_HQ_LATENT_PR05, DatasetLoader.CELEBA_HQ_LATENT,
                          DatasetLoader.CELEBA_HQ_LATENT_SYN]:
        bs = config.batch_32
    ...
    elif config.dataset in [DatasetLoader.CELEBA, DatasetLoader.CELEBA_HQ_SYN, DatasetLoader.CELEBA_HQ,
                            DatasetLoader.LSUN_CHURCH, DatasetLoader.LSUN_BEDROOM]:
        bs = config.batch_256
        if config.learning_rate == None:
            if config.ckpt == None:
    ...
    if args.mode == MODE_TRAIN or args.mode == MODE_TRAIN_MEASURE:
        ...
        setattr(config, "output_dir", os.path.join(config.result, format_ckpt_dir(args.ckpt)))
...
from diffusers import DDPMPipeline, ScoreSdeVePipeline, LDMPipeline
...
def get_latents_dataloader(config: TrainingConfig, dsl: DatasetLoader, vae):
    ...
    latent_ds = Dataset.from_dict({DatasetLoader.TARGET: target_latents, DatasetLoader.PIXEL_VALUES: poisoned_latents,
                                   DatasetLoader.IMAGE: image_latents})
...
def get_model_optim_sched(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader):
    ...
    if config.ckpt != None:
        if config.sample_ep != None and config.mode in [MODE_MEASURE, MODE_SAMPLING]:
            ...
            model, vae, noise_sched, get_pipeline = DiffuserModelSched.get_model_sched(ckpt=ep_model_path,
                                                                                       clip_sample=config.clip,
                                                                                       noise_sched_type=config.sched,
                                                                                       sde_type=config.sde_type)
        else:
            model, vae, noise_sched, get_pipeline = DiffuserModelSched.get_model_sched(ckpt=config.ckpt,
                                                                                       clip_sample=config.clip,
                                                                                       noise_sched_type=config.sched,
                                                                                       sde_type=config.sde_type)
        # optimizer = torch.optim.Adam(model.parameters(), lr=config.learning_rate)
    else:
        model, vae, noise_sched, get_pipeline = DiffuserModelSched.get_model_sched(image_size=image_size,
                                                                                   channels=channel,
                                                                                   ckpt=DiffuserModelSched.MODEL_DEFAULT,
                                                                                   noise_sched_type=config.sched,
                                                                                   clip_sample=config.clip,
                                                                                   sde_type=config.sde_type)
        # optimizer = torch.optim.Adam(model.parameters(), lr=config.learning_rate)
...
def init_train(config: TrainingConfig, dataset_loader: DatasetLoader):
    ...
    model, vae, optimizer, lr_sched, noise_sched, cur_epoch, cur_step, get_pipeline = get_model_optim_sched(
        config=config, accelerator=accelerator, dataset_loader=dataset_loader)
...
def analyze_image_channels(img):
    ...
    for i in range(3): 
        ...
        mid_val = np.mean(channel)
        channel_info.append((min_val, max_val, mid_val))

    channel_names = ['Red', 'Green', 'Blue']
    for name, (min_val, max_val, mid_val) in zip(channel_names, channel_info):
        print(f"{name} channel - Min: {min_val}, Max: {max_val}, mean: {mid_val}")
...
def sampling(config: TrainingConfig, file_name: Union[int, str], pipeline, inverted_trigger=None):
    def gen_samples(init: torch.Tensor, folder: Union[os.PathLike, str], start_from: int = 0):
        ...
        if config.ddim_eta == None:
            pipline_res = pipeline(num_inference_steps=config.infer_steps, start_from=start_from,
                                   batch_size=config.eval_sample_n, generator=torch.manual_seed(config.seed), init=init,
                                   save_every_step=True, output_type=None)
        else:
            pipline_res = pipeline(num_inference_steps=config.infer_steps, start_from=start_from, eta=config.ddim_eta,
                                   batch_size=config.eval_sample_n, generator=torch.manual_seed(config.seed), init=init,
                                   save_every_step=True, output_type=None)
        ...
        # sample_0 = [image for image in (movie[0] * 255).round().astype("uint8")]
        # analyze_image_channels(np.stack(sample_0, axis=0))
    ...
    with torch.no_grad():
        print(
            f"Sampling Init Noise -Sample_n: {config.eval_sample_n}, Channel: {pipeline.unet.in_channels}, Sample_size: {pipeline.unet.sample_size}")
        noise = torch.randn(
            (config.eval_sample_n, pipeline.unet.in_channels, pipeline.unet.sample_size, pipeline.unet.sample_size),
            generator=torch.manual_seed(config.seed),
        )
        # print("sampling: ", noise.shape)
        ...
        if config.task == TASK_GENERATE:
            ...
            # Sample Backdoor Samples with the inverted trigger
            # init = noise + torch.where(dsl.trigger.unsqueeze(0) == -1.0, 0, 1)
            if inverted_trigger is not None:
                if hasattr(pipeline, 'encode'):
                    # init = noise.to(pipeline.device) + pipeline.encode(inverted_trigger.to(pipeline.device))
                    init = noise.to(pipeline.device) + inverted_trigger.to(pipeline.device)
                else:
                    init = noise.to(pipeline.device) + inverted_trigger.to(pipeline.device)
                print(
                    f"Trigger - (max: {torch.max(dsl.trigger)}, min: {torch.min(dsl.trigger)}) | Noise - (max: {torch.max(noise)}, min: {torch.min(noise)}) | Init - (max: {torch.max(init)}, min: {torch.min(init)})")
                gen_samples(init=init, folder="backdoor_samples_inverted", start_from=0)
        else:
            # Special Sampling
            ...
            if config.task == TASK_UNPOISONED_DENOISE:
                ...
                gen_samples(init=(imgs + noise_sp) * mul, folder=f"unpoisoned_noisy_samples{ext}",
                            start_from=start_from_sp)
            elif config.task == TASK_POISONED_DENOISE:
                # Sample Poisoned Noisy Samples
                gen_samples(init=(poisoned_imgs + noise_sp) * mul, folder=f"poisoned_noisy_samples{ext}",
                            start_from=start_from_sp)
                ...
                gen_samples(init=corrupt_imgs * mul, folder=f"inpaint_box_unpoisoned_samples{ext}",
                            start_from=start_from_sp)
                ...
                gen_samples(init=corrupt_imgs * mul, folder=f"inpaint_box_poisoned_samples{ext}",
                            start_from=start_from_sp)
                ...
                gen_samples(init=corrupt_imgs * mul, folder=f"inpaint_line_unpoisoned_samples{ext}",
                            start_from=start_from_sp)
                ...
                gen_samples(init=corrupt_imgs * mul, folder=f"inpaint_line_poisoned_samples{ext}",
                            start_from=start_from_sp)
            else:
...
def update_score_file(config: TrainingConfig, score_file: str, fid_sc: float = None, mse_sc: float = None,
                      ssim_sc: float = None, asr_sc: float = None, lpips_sc: float = None, mse_thres_sc=None) -> Dict:
    ...
    if config.task == TASK_GENERATE:
        if fid_sc == None or mse_sc == None or ssim_sc == None or asr_sc == None:
            raise ValueError(f"Task: {config.task} requires FID, MSE, and SSIM scores")
        # get_key_fn = get_key_gen
        scores = {"FID": fid_sc, "MSE": mse_sc, "SSIM": ssim_sc, "ASR": asr_sc}
    else:
    ...
    try:
        ...
        print(os.path.join(config.output_dir, score_file))
...
def gen_batch_samples(pipeline, init: torch.Tensor, num_inference_steps: int, start_from: int, batch_size: int,
                      seed: int = 1):
    # Sample some images from random noise (this is the backward diffusion process).
    # The default pipeline output type is `List[PIL.Image]`
...
def gen_samples(pipeline, sample_n: int, init: torch.Tensor = None, num_inference_steps: int = 1000,
                start_from: int = 0, max_batch_n: int = 256, seed: int = 1):
    batch_sizes, init = get_bs_inits(sample_n=sample_n, init=init, max_batch_n=max_batch_n)

    sample_imgs_ls = []
    for i, batch_sz in enumerate(batch_sizes):
        if init != None:
            samples = gen_batch_samples(pipeline, init=init[i], num_inference_steps=num_inference_steps,
                                        start_from=start_from, batch_size=batch_sz, seed=seed)
        else:
            samples = gen_batch_samples(pipeline, init=None, num_inference_steps=num_inference_steps,
                                        start_from=start_from, batch_size=batch_sz, seed=seed)
...
def measure_inpaint(config: TrainingConfig, pipeline, target_imgs: torch.Tensor, corrupt_imgs: torch.Tensor):
    ...
    recover_imgs = gen_samples(pipeline=pipeline, sample_n=len(target_imgs), init=corrupt_imgs * mul,
                               num_inference_steps=config.infer_steps, start_from=start_from_sp,
                               max_batch_n=config.eval_max_batch, seed=config.seed)
...
def measure_inpaints(config: TrainingConfig, pipeline, dsl: DatasetLoader) -> Tuple[float, float, float]:
    device = torch.device(config.device_ids[0])
    noise = torch.randn(
        (config.measure_inpaint_sample_n, pipeline.unet.in_channels, pipeline.unet.sample_size,
    ...
    if config.task == TASK_UNPOISONED_DENOISE:
        ...
        sc = measure_inpaint(config=config, pipeline=pipeline, target_imgs=backdoor_targets,
                             corrupt_imgs=poisoned_imgs + noise_sp)
    elif config.task == TASK_UNPOISONED_INPAINT_LINE:
...
def measure(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader,
            folder_name: Union[int, str], pipeline,
            num_clean=10000, num_backdoor=16, resample: bool = False, recomp: bool = True):
    ...
    # Folders
    if config.dataset == "CIFAR10" or config.dataset == "CIFAR10-SYN":
        dataset_img_dir = "D:\\00_dataset\\cifar10.npz"
        config.eval_max_batch = 128
    elif config.dataset == "CELEBA-HQ" or config.dataset == "CELEBA-HQ-SYN" or config.dataset == "CELEBA-HQ-LATENT" or config.dataset == "CELEBA-HQ-LATENT-SYN":
        dataset_img_dir = "D:\\00_dataset\\celebahq256.npz"
        config.eval_max_batch = 8
    ...
    if config.task != TASK_GENERATE:
        ...
        # Sampling
        if not os.path.isdir(clean_path) or match_count(dir=clean_path) < num_clean or resample:
            print("num_clean: ", num_clean, config.eval_max_batch)
            batch_sampling_save(sample_n=num_clean, num_inference_steps=config.infer_steps, ddim_eta=config.ddim_eta,
                                pipeline=pipeline,
                                path=clean_path, max_batch_n=config.eval_max_batch, rng=rng)
        ...
        if not os.path.isdir(backdoor_path) or match_count(dir=backdoor_path) < num_backdoor or resample:
            ...
            batch_sampling_save(sample_n=num_backdoor, num_inference_steps=config.infer_steps, ddim_eta=config.ddim_eta,
                                pipeline=pipeline,
                                path=backdoor_path, trigger=dataset_loader.trigger.unsqueeze(0),
        ...
        if re_comp_clean_metric or recomp:
            fid_sc = float(fid(path=[dataset_img_dir, clean_path], device=config.device_ids[0], num_workers=1,
                               batch_size=config.eval_max_batch))
...
def deshift_loss(model, noise, trigger, T, frozen_model, config):
    # TODO: update the code for score-based models.
    if config.sde_type in [DiffuserModelSched.SDE_VP, DiffuserModelSched.SDE_LDM]:
...
def train_loop(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, get_pipeline, noise_sched,
               optimizer: torch.optim, loader, lr_sched, vae=None, start_epoch: int = 0, start_step: int = 0,
    ...
    try:
        ...
        loss_fn = LossFn(noise_sched=noise_sched, sde_type=config.sde_type, loss_type="l2", psi=config.psi,
                         solver_type=config.solver_type, vp_scale=config.vp_scale, ve_scale=config.ve_scale)

        pipeline = get_pipeline(accelerator, model, vae, noise_sched)
        # print("main", noise_sched.config.num_train_timesteps, len(noise_sched.sigmas))
        sampling(config, -1, pipeline, inverted_trigger)
        # Now you train the model

        deshift_iters = 0
        # if type(pipeline) == ScoreSdeVePipeline:
        #    noise_sched.set_sigmas(num_inference_steps=noise_sched.config.num_train_timesteps)
        ...
        for epoch in range(int(start_epoch), int(config.epoch)):
            ...
            for step, batch in enumerate(loader):
                if type(pipeline) == LDMPipeline and step == 375:
                    break
                ...
                timesteps = torch.randint(0, noise_sched.config.num_train_timesteps, (bs,),
                                          device=clean_images.device).long()
                # print(f"time steps used: {timesteps}")
                ...
                with accelerator.accumulate(model):
                    ...
                    if inverted_trigger is not None:  # and deshift_iters < 20:
                        deshift_iters += 1
                        loss1, loss2 = deshift_loss(model, noise, inverted_trigger, noise_sched.config.num_train_timesteps - 1, frozen_model, config)  # changed for ddim scheduler, others should also work
...
if __name__ == '__main__':
    ...
    os.environ["VECLIB_MAXIMUM_THREADS"] = "1"  # Accelerate Framework
    ...
    if config.mode == MODE_TRAIN or config.mode == MODE_RESUME or config.mode == MODE_TRAIN_MEASURE:
        inverted_trigger = None
        R_coef_T = 0.5
        # trigger_filename = f'./elijah_helper_baddiff/inverted_trigger/{format_ckpt_dir(config.ckpt)}_trigger_{R_coef_T}.pt'
        # trigger_filename = f'./elijah_helper_ddim/inverted_trigger/{format_ckpt_dir(config.ckpt)}_trigger_{R_coef_T}.pt'
        trigger_filename = f'./elijah_helper_ncsn/inverted_trigger/{format_ckpt_dir(config.ckpt)}_trigger_{R_coef_T}.pt'
        # trigger_filename = f'./elijah_helper_ldm/inverted_trigger/{format_ckpt_dir(config.ckpt)}_trigger_{R_coef_T}.pt'
        if config.remove_backdoor:
            if os.path.isfile(trigger_filename):
                ...
                print(f"inverted trigger: ", inverted_trigger.shape)
            else:
        ...
        pipeline = train_loop(config, accelerator, repo, model, get_pipeline, noise_sched, optimizer, dataloader,
                              lr_sched, vae=vae, start_epoch=cur_epoch, start_step=cur_step,
                              inverted_trigger=inverted_trigger)

        if config.mode == MODE_TRAIN_MEASURE and accelerator.is_main_process:
            accelerator.free_memory()
            accelerator.clear()
            measure(config=config, accelerator=accelerator, dataset_loader=dsl, folder_name='measure',
                    pipeline=pipeline)
    elif config.mode == MODE_SAMPLING:
        # pipeline = DDPMPipeline(unet=accelerator.unwrap_model(model), scheduler=noise_sched)
        pipeline = get_pipeline(accelerator, model, vae, noise_sched)
