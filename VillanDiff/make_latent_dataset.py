...
class TrainingConfig:
    latent_dataset_dir: str = 'celeba_hq_256_latents_syn'
    dataset_name: str = DatasetLoader.CELEBA_HQ_SYN
...
def get_model_optim_sched(config: TrainingConfig):  # init a model based on the config
...
def get_latents_dataset(config: TrainingConfig, dsl: DatasetLoader, vae):
    ...
    with torch.no_grad():
        ...
        poisoned_latents = torch.cat([encode_latents(x=batch[DatasetLoader.PIXEL_VALUES]) for batch in tqdm(dl)], dim=0)  # poisoned images after VAE
        image_latents = torch.cat([encode_latents(x=batch[DatasetLoader.IMAGE]) for batch in tqdm(dl)], dim=0)  # clean images after VAE
        latent_ds: Dataset = Dataset.from_dict(
            {DatasetLoader.TARGET: target_latents, DatasetLoader.PIXEL_VALUES: poisoned_latents,
...
class LatentDataset(torch.utils.data.Dataset):
    ...
    def targe_latents_path(self):
        p = os.path.join(self.__ds_root, LatentDataset.TARGET_LATENTS_FILE_NAME+"_"+config.target)
...
def generate_latents_dataset(config: TrainingConfig, lds: LatentDataset, dsl: DatasetLoader):
    """transfer images from dsl to latents dataset using VAE"""
...
def main(config: TrainingConfig, is_raw: bool):
    ...
    dsl = DatasetLoader(root=ds_root, name=config.dataset_name, batch_size=config.batch_size, shuffle=False).set_poison(
        trigger_type=config.trigger, target_type=config.target, clean_rate=1.0,
        poison_rate=config.poison_rate).prepare_dataset(mode=DatasetLoader.MODE_FIXED)
    ...
    lds: LatentDataset = LatentDataset(ds_root=os.path.join(ds_root, config.latent_dataset_dir))  # change to syn
    lds.set_vae(vae=vae).set_poison(target_key=config.target, poison_key=config.trigger, raw='raw',
                                    poison_rate=config.poison_rate, use_latent=True).set_use_names(
