...
class DatasetLoader(object):
    ...
    def __init__(self, name: str, label: int = None, root: str = None, channel: int = None, image_size: int = None,
                 vmin: Union[int, float] = DEFAULT_VMIN, vmax: Union[int, float] = DEFAULT_VMAX, batch_size: int = 512,
    ...
    def set_poison(self, trigger_type: str, target_type: str, target_dx: int = -5, target_dy: int = -3,
                   clean_rate: float = 1.0, poison_rate: float = 0.2, ext_poison_rate: float = 0.0) -> 'DatasetLoader':
        if self.__root == None:
        ...
        self.__trigger = self.__backdoor.get_trigger(type=trigger_type, channel=self.__channel,
                                                     image_size=self.__image_size, vmin=self.__vmin, vmax=self.__vmax)
    ...
    def __load_dataset(self, name: str):
        ...
        if name == DatasetLoader.MNIST:
            ...
            # return load_dataset("D:\\00_dataset\\cifar10", split=split_method)
            return load_dataset("googla/cifar10", split=split_method)
        elif name == DatasetLoader.CIFAR10_SYN:
            return load_dataset("D:\\00_dataset\\cifar10_syn", data_dir="D:\\00_dataset\\cifar10_syn", split="train")  # images generated by backdoored models, organized as dataset files
            ...
            # return load_dataset("D:\\00_dataset\\celebahq-1024", split="train+validation")  # images generated by backdoored models, organized as dataset files
        elif name == DatasetLoader.CELEBA_HQ_SYN:
            dataset_tmp = load_dataset("D:\\00_dataset\\celebahq256-syn", split="train")
            # print(dataset_tmp[0])
            # print(dataset_tmp[0]["image"])
            return dataset_tmp
        ...
        #     return load_from_disk("datasets/celeba_hq_256_pr05")
            ...
            return LatentDataset(ds_root="G:\\celeba_hq_256_latents")  # made by "make_latent_dataset.py"
        elif name == DatasetLoader.CELEBA_HQ_LATENT_SYN:  # images generated by backdoored LDM, and translated to latents
            # dataset_tmp = LatentDataset(ds_root="G:\\celeba_hq_256_latents_syn")
            # print("dataset: ", len(dataset_tmp))
            return LatentDataset(ds_root="G:\\celeba_hq_256_latents_syn")
        else:
    ...
    def __set_img_shape(self, image_size: int) -> None:
        ...
        if self.__name == self.MNIST:
            ...
            # self.__vmax = 1
            self.__cmap = "gray"
        elif self.__name == self.CIFAR10 or self.__name == self.CELEBA or self.__name == self.CELEBA_HQ or self.__name == self.CIFAR10_SYN or self.__name == self.CELEBA_HQ_SYN or self.__name == self.LSUN_CHURCH or self.__name == self.CELEBA_HQ_LATENT_PR05 or self.__name == self.CELEBA_HQ_LATENT or self.__name == self.CELEBA_HQ_LATENT_SYN:
        ...
        if image_size == None:
            ...
            elif self.__name == self.CIFAR10 or self.__name == self.CIFAR10_SYN:
                self.__image_size = 32
            elif self.__name == self.CELEBA:
                self.__image_size = 64
            elif self.__name == self.CELEBA_HQ or self.__name == self.CELEBA_HQ_SYN or self.__name == self.LSUN_CHURCH or self.__name == self.CELEBA_HQ_LATENT_PR05 or self.__name == self.CELEBA_HQ_LATENT or self.__name == self.CELEBA_HQ_LATENT_SYN:
    ...
    def __flex_sz_dataset_old(self):
        ...
        if self.__poison_rate > 0:
            ...
            self.__poison_n = int(full_ds_len * float(self.__poison_rate))
            self.__backdoor_dataset = Subset(self.__backdoor_dataset[DatasetLoader.TRAIN],
                                             list(range(0, self.__poison_n, 1)))
            self.__full_dataset[DatasetLoader.TRAIN] = ConcatDataset(
                [self.__full_dataset[DatasetLoader.TRAIN], self.__backdoor_dataset])
    ...
    def __flex_sz_dataset(self):
        ...
        if clean_ds is not None:
            ...
            ds_ls.append(
                clean_ds.add_column(DatasetLoader.IS_CLEAN, [True] * portion_sz(rate=self.__clean_rate, n=ds_n)))
        else:
        ...
        if backdoor_ds is not None:
            ...
            ds_ls.append(
                backdoor_ds.add_column(DatasetLoader.IS_CLEAN, [False] * portion_sz(rate=self.__poison_rate, n=ds_n)))
        else:
        ...
        self.__full_dataset = concatenate_datasets(ds_ls)  # concate the clean and backdoor dataset
        self.__full_dataset = self.__full_dataset.with_transform(trans)
    ...
    def __extend_sz_dataset(self):
        ...
        if clean_dataset != None:
            ...
            clean_dataset = clean_dataset.add_column(DatasetLoader.IS_CLEAN, [True] * clean_n).add_column(
                DatasetLoader.R_trigger_only, [False] * clean_n)
        ...
        if ext_backdoor_dataset != None:
            ...
            ext_backdoor_dataset = ext_backdoor_dataset.add_column(DatasetLoader.IS_CLEAN,
                                                                   [False] * ext_backdoor_n).add_column(
                DatasetLoader.R_trigger_only, [self.__ext_R_trigger_only] * ext_backdoor_n)
            print(
                f"[Mode Extend] Extend Backdoor Dataset Size: {len(ext_backdoor_dataset)},  {ext_backdoor_dataset[1].keys()}")
        ...
        if backdoor_dataset is not None:
            ...
            backdoor_dataset = backdoor_dataset.add_column(DatasetLoader.IS_CLEAN, [False] * backdoor_n).add_column(
                DatasetLoader.R_trigger_only, [self.__R_trigger_only] * backdoor_n)
    ...
    def prepare_dataset(self, mode: str = "FIXED", R_trigger_only: bool = False, ext_R_trigger_only: bool = False,
                        R_gaussian_aug: float = 0.0) -> 'DatasetLoader':
        ...
        if self.__name == self.CELEBA_HQ_LATENT or self.__name == self.CELEBA_HQ_LATENT_SYN:
            self.__full_dataset.set_poison(target_key=self.__target_type, poison_key=self.__trigger_type, raw='raw',
                                           poison_rate=self.__poison_rate, use_latent=True).set_use_names(
                target=DatasetLoader.TARGET, poison=DatasetLoader.PIXEL_VALUES, raw=DatasetLoader.IMAGE)
        ...
        if len(ex) == 1:
            ...
            print(
                f"Note that CHANNEL 0 - vmin: {torch.min(ex[0])} and vmax: {torch.max(ex[0])} | CHANNEL 1 - vmin: {torch.min(ex[1])} and vmax: {torch.max(ex[1])} | CHANNEL 2 - vmin: {torch.min(ex[2])} and vmax: {torch.max(ex[2])}")
        return self
    ...
    def get_dataloader(self, batch_size: int = None, shuffle: bool = None, num_workers: int = 0,
        ...
        if num_workers == None:
            num_workers = 0
        if collate_fn != None:
            return DataLoader(datasets, batch_size=batch_size, shuffle=shuffle, pin_memory=True,
    ...
    def __transform_generator(self, dataset_name: str, clean: bool, R_trigger_only: bool) -> Callable[
        ...
        if dataset_name == self.CELEBA:
            img_key = "image"
        if dataset_name == self.CELEBA_HQ or dataset_name == self.CELEBA_HQ_SYN or dataset_name == self.CIFAR10_SYN:
            img_key = "image"
        ...
        def backdoor_transforms(examples) -> DatasetDict:
            ...
            examples[DatasetLoader.PIXEL_VALUES_TRIGGER] = self.__trigger.repeat(*repeat_times)  # only contain the trigger
            if R_trigger_only:
                examples[DatasetLoader.PIXEL_VALUES] = self.__trigger.repeat(*repeat_times)
            else:
                examples[DatasetLoader.PIXEL_VALUES] = masks * examples[DatasetLoader.IMAGE] + (
                            1 - masks) * self.__trigger.repeat(*repeat_times)  # stamp triggers to the oritinal images
    ...
    def show_sample(self, img: torch.Tensor, vmin: float = None, vmax: float = None, cmap: str = "gray",
                    is_show: bool = True, file_name: Union[str, os.PathLike] = None, is_axis: bool = False) -> None:
...
class Backdoor():
    ...
    def __get_transform(self, channel: int, image_size: Union[int, Tuple[int]], vmin: Union[float, int],
                        vmax: Union[float, int], prev_trans: List = [], next_trans: List = []):
        if channel == 1:
            channel_trans = transforms.Grayscale(num_output_channels=1)
        elif channel == 3:
            channel_trans = transforms.Lambda(lambda x: x.convert("RGB"))
    ...
    def __get_img_target(self, path: Union[str, os.PathLike], image_size: int, channel: int, vmin: Union[float, int],
    ...
    def __get_img_trigger(self, path: Union[str, os.PathLike], image_size: int, channel: int, trigger_sz: int,
                          vmin: Union[float, int], vmax: Union[float, int], x: int = None, y: int = None):
        # Padding of Left & Top
        ...
        trig = self.__get_transform(channel=channel, image_size=trigger_sz, vmin=vmin, vmax=vmax,
                                    next_trans=next_trans)(img)
    ...
    @staticmethod
    def __roll(x: torch.Tensor, dx: int, dy: int):  # shift the image in the last two demensions
        shift = tuple([0] * len(x.shape[:-2]) + [dy] + [dx])
        dim = tuple([i for i in range(len(x.shape))])
        return torch.roll(x, shifts=shift, dims=dim)

    @staticmethod
    def __get_box_trig(b1: Tuple[int, int], b2: Tuple[int, int], channel: int, image_size: int, vmin: Union[float, int],
                       vmax: Union[float, int], val: Union[float, int]):
    ...
    @staticmethod
    def __get_white_box_trig(b1: Tuple[int, int], b2: Tuple[int, int], channel: int, image_size: int,
                             vmin: Union[float, int], vmax: Union[float, int]):
        return Backdoor.__get_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin, vmax=vmax,
                                       val=vmax)

    @staticmethod
    def __get_grey_box_trig(b1: Tuple[int, int], b2: Tuple[int, int], channel: int, image_size: int,
                            vmin: Union[float, int], vmax: Union[float, int]):
        return Backdoor.__get_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin, vmax=vmax,
                                       val=(vmin + vmax) / 2)
    ...
    def __get_trig_box_coord(x: int, y: int):
        ...
        return (- (y + Backdoor.TRIGGER_GAP_Y), - (x + Backdoor.TRIGGER_GAP_X)), (
        - Backdoor.TRIGGER_GAP_Y, - Backdoor.TRIGGER_GAP_X)

    def get_trigger(self, type: str, channel: int, image_size: int, vmin: Union[float, int] = DEFAULT_VMIN,
                    vmax: Union[float, int] = DEFAULT_VMAX) -> torch.Tensor:
        if type == Backdoor.TRIGGER_FA:
            ...
            # return trig
            return Backdoor.__get_white_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin,
                                                 vmax=vmax)
            ...
            b1, b2 = Backdoor.__get_trig_box_coord(18, 18)
            return Backdoor.__get_grey_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin,
                                                vmax=vmax)
            ...
            return self.__get_img_trigger(path=Backdoor.GLASSES_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=trigger_sz, vmin=vmin, vmax=vmax)
        elif type == Backdoor.TRIGGER_BIG_STOP_SIGN:
            return self.__get_img_trigger(path=Backdoor.STOP_SIGN_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=18, vmin=vmin, vmax=vmax, x=-2, y=-2)
        elif type == Backdoor.TRIGGER_SM_STOP_SIGN or type == "SM_STOP_SIGN":
            return self.__get_img_trigger(path=Backdoor.STOP_SIGN_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=14, vmin=vmin, vmax=vmax, x=-2, y=-2)
        elif type == Backdoor.TRIGGER_XSM_STOP_SIGN:
            return self.__get_img_trigger(path=Backdoor.STOP_SIGN_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=11, vmin=vmin, vmax=vmax, x=-2, y=-2)
        elif type == Backdoor.TRIGGER_XXSM_STOP_SIGN:
            return self.__get_img_trigger(path=Backdoor.STOP_SIGN_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=8, vmin=vmin, vmax=vmax, x=-2, y=-2)
        elif type == Backdoor.TRIGGER_XXXSM_STOP_SIGN:
            return self.__get_img_trigger(path=Backdoor.STOP_SIGN_IMG, image_size=image_size, channel=channel,
                                          trigger_sz=4, vmin=vmin, vmax=vmax, x=-2, y=-2)
    ...
    def __check_channel(self, sample: torch.Tensor, channel_first: bool = None) -> int:
        ...
        if (sample.shape[Backdoor.CHANNEL_LAST] == 1 or sample.shape[Backdoor.CHANNEL_LAST] == 3) and \
            ...
            raise ValueError(
                f"Duplicate channel found, found {sample.shape[Backdoor.CHANNEL_LAST]} at dimension 2 and {sample.shape[Backdoor.CHANNEL_FIRST]} at dimension 0")
        ...
        if sample.shape[Backdoor.CHANNEL_LAST] == 1 or sample.shape[Backdoor.CHANNEL_LAST] == 3:
            ...
            raise ValueError(
                f"Invalid channel shape, found {sample.shape[Backdoor.CHANNEL_LAST]} at dimension 2 and {sample.shape[Backdoor.CHANNEL_FIRST]} at dimension 0")
    ...
    def get_target(self, type: str, trigger: torch.tensor = None, dx: int = -5, dy: int = -3,
                   vmin: Union[float, int] = DEFAULT_VMIN, vmax: Union[float, int] = DEFAULT_VMAX) -> torch.Tensor:
        ...
        if type == Backdoor.TARGET_TG or type == Backdoor.TARGET_TRIGGER:
            if trigger == None:
        ...
        elif type == Backdoor.TARGET_WHITE_BOX:
            ...
            b1 = (None, None)
            b2 = (10, 10)
            return Backdoor.__bg2grey(
                trig=Backdoor.__get_grey_box_trig(b1=b1, b2=b2, channel=channel, image_size=image_size, vmin=vmin,
            ...
            # trig = self.__get_transform(channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)(img)
            # return trig
            return self.__get_img_target(path="static/hat.png", channel=channel, image_size=image_size, vmin=vmin,
                                         vmax=vmax)
        elif type == Backdoor.TARGET_FEDORA_HAT or type == "HAT":
            # img = Backdoor.__read_img("static/fedora-hat.png")
            # trig = self.__get_transform(channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)(img)
            # return trig
            return self.__get_img_target(path="static/fedora-hat.png", channel=channel, image_size=image_size,
                                         vmin=vmin, vmax=vmax)
        elif type == Backdoor.TARGET_CAT:
            # img = Backdoor.__read_img("static/cat.png")
            # trig = self.__get_transform(channel=channel, image_size=image_size, vmin=vmin, vmax=vmax)(img)
            # return trig
            return self.__get_img_target(path=Backdoor.CAT_IMG, channel=channel, image_size=image_size, vmin=vmin,
                                         vmax=vmax)
        else:
...
class LatentDataset(torch.utils.data.Dataset):
    DATA_EXT: str = "pt"
    TARGET_LATENTS_FILE_NAME: str = f"target_CAT"
    # POISON_LATENTS_FILE_NAME: str = f"poison"
    POISON_LATENTS_FILE_NAME: str = f"GLASSES"
    ...
    @staticmethod
    def __encode_latents_static(x: torch.Tensor, vae, weight_dtype: str = None,
                                scaling_factor: float = None) -> torch.Tensor:
        vae = vae.eval()
        with torch.no_grad():
            x = x.to(vae.device)
    ...
    @staticmethod
    def __decode_latents_static(vae, x: torch.Tensor, weight_dtype: str = None,
                                scaling_factor: float = None) -> torch.Tensor:
        vae = vae.eval()
        with torch.no_grad():
            x = x.to(vae.device)
    ...
    def __encode_latents(self, x: torch.Tensor, weight_dtype: str = None, scaling_factor: float = None) -> torch.Tensor:
        ...
        return LatentDataset.__encode_latents_static(vae=self.__vae, x=x, weight_dtype=weight_dtype,
                                                     scaling_factor=scaling_factor)

    def __decode_latents(self, x: torch.Tensor, weight_dtype: str = None, scaling_factor: float = None) -> torch.Tensor:
        if self.__vae == None:
            raise ValueError("Please provide encoder first")
        return LatentDataset.__decode_latents_static(vae=self.__vae, x=x, weight_dtype=weight_dtype,
                                                     scaling_factor=scaling_factor)
    ...
    def __update_dict_key(self, file: Union[str, os.PathLike], key: str, val: torch.Tensor) -> None:
        LatentDataset.__update_dict_key_latent(file=file, key=key,
                                               val=self.__encode_latents(x=val.unsqueeze(0)).squeeze(0))
    ...
    def __getitem__(self, i: int):
        ...
        if self.__use_latent:
            return {
                self.__used_target_name: clean_poison(self.get_raw_latents_by_idxs,
                                                      lambda idxs: self.get_target_latent())(i),
                self.__used_poison_name: clean_poison(zeros_like(self.get_raw_latents_by_idxs(idxs=i)),
                                                      self.get_poison_latents_by_idxs)(i),
            ...
            return {
                ...
                self.__used_poison_name: clean_poison(zeros_like(self.get_raw_by_idxs(idxs=i)),
                                                      self.get_poison_by_idxs)(i),
...
if __name__ == "__main__":
    ...
    dsl = DatasetLoader(root=ds_root, name=DatasetLoader.CELEBA_HQ, batch_size=128).set_poison(
        trigger_type=Backdoor.TRIGGER_GLASSES, target_type=Backdoor.TARGET_CAT, clean_rate=0.2,
        poison_rate=0.4).prepare_dataset(mode=DatasetLoader.MODE_FIXED)
    ...
    print(
        f"Clean Target: {sample['target'].shape} | Label: {sample['label']}  | pixel_values: {sample['pixel_values'].shape}")
    print(
        f"Clean PIXEL_VALUES Shape: {sample['pixel_values'].shape} | vmin: {torch.min(sample['pixel_values'])} | vmax: {torch.max(sample['pixel_values'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['pixel_values'])
    print(
        f"Clean TARGET Shape: {sample['target'].shape} | vmin: {torch.min(sample['target'])} | vmax: {torch.max(sample['target'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['target'])
    print(
        f"Clean IMAGE Shape: {sample['image'].shape} | vmin: {torch.min(sample['image'])} | vmax: {torch.max(sample['image'])} | CLEAN: {sample['is_clean']}")
    ...
    print(f"Full Dataset Len: {len(train_ds)} | Sample Len: {len(sample)}")
    print(
        f"Backdoor Target: {sample['target'].shape} | Label: {sample['label']}  | pixel_values: {sample['pixel_values'].shape}")
    print(
        f"Backdoor PIXEL_VALUES Shape: {sample['pixel_values'].shape} | vmin: {torch.min(sample['pixel_values'])} | vmax: {torch.max(sample['pixel_values'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['pixel_values'])
    print(
        f"Backdoor TARGET Shape: {sample['target'].shape} | vmin: {torch.min(sample['target'])} | vmax: {torch.max(sample['target'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['target'])
    print(
        f"Backdoor Noisy PIXEL_VALUES Shape: {sample['pixel_values'].shape} | vmin: {torch.min(sample['pixel_values'])} | vmax: {torch.max(sample['pixel_values'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['pixel_values'] + noise)
    print(
        f"Backdoor IMAGE Shape: {sample['image'].shape} | vmin: {torch.min(sample['image'])} | vmax: {torch.max(sample['image'])} | CLEAN: {sample['is_clean']}")
    dsl.show_sample(sample['image'])
    ...
    plt.tight_layout()
    # SM_BOX_MED Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_SM_STOP_SIGN, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax)
    backdoor.show_image(img=tr)
    # plt.imshow(torch.squeeze(tr), cmap='gray', vmin=vmin, vmax=vmax)
    # plt.show()
    # SM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_SM_BOX_MED, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax)
    backdoor.show_image(img=tr)
    # plt.imshow(torch.squeeze(tr), cmap='gray', vmin=vmin, vmax=vmax)
    # plt.show()
    # XSM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_XSM_BOX_MED, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax).permute(1, 2, 0)
    plt.imshow(torch.squeeze(tr), cmap='gray', vmin=vmin, vmax=vmax)
    plt.show()
    # XXSM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_XXSM_BOX_MED, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax).permute(1, 2, 0)
    plt.imshow(torch.squeeze(tr), cmap='gray', vmin=vmin, vmax=vmax)
    plt.show()
    # XXXSM_BOX Trigger
    tr = backdoor.get_trigger(type=Backdoor.TRIGGER_XXXSM_BOX_MED, channel=channel, image_size=image_size, vmin=vmin,
                              vmax=vmax).permute(1, 2, 0)
    plt.imshow(torch.squeeze(tr), cmap='gray', vmin=vmin, vmax=vmax)
    plt.show()
    # GLASSES Trigger
