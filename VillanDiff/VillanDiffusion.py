...
DEFAULT_LEARNING_RATE_256: float = 2e-5
...
DEFAULT_POSTFIX: str = ""
DEFAULT_FCLIP: str = 'w'
DEFAULT_SAVE_IMAGE_EPOCHS: int = 1
DEFAULT_SAVE_MODEL_EPOCHS: int = 10
...
MODE_RESUME_OPTS = ['project', 'task', 'sched', 'ddim_eta', 'infer_steps', 'mode', 'gpu', 'ckpt', 'seed']
MODE_SAMPLING_OPTS = ['project', 'task', 'sched', 'ddim_eta', 'infer_steps', 'mode', 'eval_max_batch', 'gpu', 'fclip',
                      'ckpt', 'sample_ep', 'infer_start', 'inpaint_mul', 'seed']
...
def parse_args():
    ...
    parser.add_argument('--seed', type=int, help=f"random seed, default: {0}", default=0)
...
class TrainingConfig:
    ...
    eval_sample_n: int = 8  # how many images to sample during evaluation
    ...
    learning_rate_256_fine: float = 1e-5
...
def naming_fn(config: TrainingConfig):
    ...
    # return f'res_{config.ckpt}_{config.dataset}_ep{config.epoch}_{config.solver_type}_c{config.clean_rate}_p{config.poison_rate}_epr{config.ext_poison_rate}_psi{config.psi}_lr{config.learning_rate}_seed{config.seed}'
    return f'res_{config.ckpt}_{config.dataset}_ep{config.epoch}_{config.solver_type}_c{config.clean_rate}_p{config.poison_rate}_epr{config.ext_poison_rate}_{config.trigger}-{config.target}'
...
def setup():
    ...
    if args.mode == MODE_TRAIN or args.mode == MODE_TRAIN_MEASURE:
        setattr(config, "output_dir", os.path.join(config.result, naming_fn(config=config)))  # output_dir is computed using naming_fn()
...
def get_latents_dataloader(config: TrainingConfig, dsl: DatasetLoader, vae):
    ...
    return DataLoader(latent_ds, batch_size=config.batch, shuffle=True, pin_memory=True, num_workers=1)
...
def init_train(config: TrainingConfig, dataset_loader: DatasetLoader):
    ...
    if accelerator.is_main_process:
        # if config.push_to_hub:
        #     repo = init_git_repo(config, at_init=True)
        if "\\" in config.output_dir:
            tracker_project: str = str(config.output_dir).split('\\')[-1]
        elif "/" in config.output_dir:
            tracker_project: str = str(config.output_dir).split('/')[-1]
        else:
            tracker_project: str = str(config.output_dir)
        ...
        # print(f"tracker_project: {tracker_project}")  # res_DDPM-CIFAR10-32_CIFAR10_ep100_sde_c1.0_p0.3_epr0.0_STOP_SIGN_4-SHOE_psi1_lr0.0002_vp1.0_ve1.0
...
def update_score_file(config: TrainingConfig, score_file: str, fid_sc: float = None, mse_sc: float = None,
                      ssim_sc: float = None, asr_sc: float = None, lpips_sc: float = None) -> Dict:
...
def measure(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader,
            ...
            num_clean=10000, num_backdoor=1000, resample: bool = False, recomp: bool = True):
            # num_clean = 16, num_backdoor = 16, resample: bool = False, recomp: bool = True):
    ...
    def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], trigger: torch.Tensor = None,
        ...
        print(f"batch_sampling_save: {sample_n}")
        cnt = 0
        for i, batch_sz in enumerate(batch_sizes):
            noise = torch.randn(
    ...
    elif config.dataset == "CELEBA-HQ-LATENT":
        dataset_img_dir = "D:\\00_dataset\\celebahq256.npz"
        config.eval_max_batch = 8
    ...
    if config.task != TASK_GENERATE:
        ...
        if not os.path.isdir(clean_path) or match_count(dir=clean_path) < num_clean or resample:
            ...
            batch_sampling_save(sample_n=num_clean-match_count(dir=clean_path), num_inference_steps=config.infer_steps, ddim_eta=config.ddim_eta,
                                pipeline=pipeline,
...
def checkpoint(config: TrainingConfig, accelerator: Accelerator, pipeline, cur_epoch: int, cur_step: int, repo=None,
    ...
    # print("checkpoint: ", config.ckpt_path, "  data path: ", config.data_ckpt_path)
...
def train_loop(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, get_pipeline, noise_sched,
    ...
    try:
        ...
        for epoch in range(int(start_epoch), int(config.epoch)):
            ...
            for step, batch in enumerate(loader):
                ...
                with accelerator.accumulate(model):
                    ...
                    loss = loss_fn.p_loss_by_keys(batch=batch, model=model, vae=None, target_latent_key="target",
                                                  poison_latent_key="pixel_values", timesteps=timesteps, noise=None,
                                                  weight_dtype=weight_dtype, scaling_factor=scaling_factor)
            ...
            if accelerator.is_main_process:
                ...
                if (epoch + 1) % config.save_image_epochs == 0 or epoch == config.epoch - 1:
                    print("sampling in training")
                    sampling(config, epoch, pipeline)

                if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.epoch - 1:
                    print("save model in training")
                    checkpoint(config=config, accelerator=accelerator, pipeline=pipeline, cur_epoch=epoch,
        ...
        pipeline = get_pipeline(accelerator, model, vae, noise_sched)
        if accelerator.is_main_process:
            checkpoint(config=config, accelerator=accelerator, pipeline=pipeline, cur_epoch=epoch, cur_step=cur_step,
                       repo=repo, commit_msg=f"Epoch {epoch}")
...
if config.mode == MODE_TRAIN or config.mode == MODE_RESUME or config.mode == MODE_TRAIN_MEASURE:
    pipeline = train_loop(config, accelerator, repo, model, get_pipeline, noise_sched, optimizer, dataloader, lr_sched,
                          vae=vae, start_epoch=cur_epoch, start_step=cur_step)
