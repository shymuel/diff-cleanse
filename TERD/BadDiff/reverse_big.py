    ...
    for _ in tqdm1.tqdm(
        ...
        with torch.no_grad():
            generate_image = pipeline(batch_size=args.batch_size, generator=None,
                init=x, output_type=None, num_inference_steps=args.num_steps)
...
if config.sched == "DDPM-SCHED":
    pipeline = diffusers.DDPMPipeline(unet=accelerator.unwrap_model(model), scheduler=ddpm_noise_sched)
else:
    pipeline = diffusers.DDIMPipeline(unet=accelerator.unwrap_model(model), scheduler=ddim_noise_sched)
measure(config=config, dataset_loader=dsl, folder_name='measure', pipeline=pipeline)
