...
from diffusers import UNet2DModel, DDPMScheduler, DDPMPipeline,DDIMScheduler,DDIMPipeline, ScoreSdeVePipeline, ScoreSdeVeScheduler
# from diffusers.scheduling_utils import SchedulerMixin
from diffusers import DiffusionPipeline, LDMPipeline, VQModel
...
def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], init: torch.Tensor=None, max_batch_n: int=256, rng: torch.Generator=None, args= None):
    if init == None:
        if sample_n > max_batch_n:
            replica = sample_n // max_batch_n
            residual = sample_n % max_batch_n
...
class DiffuserModelSched():
    ...
    def __get_model_sched(ckpt_id: str, clip_sample: bool):
        ...
        # need revise
        pipline: DDIMPipeline = DDIMPipeline.from_pretrained(ckpt_id)
    ...
    @staticmethod
    def __get_model_sched_ve(ckpt_id: str, clip_sample: bool):
        # Clip option
        clip_sample_used = DiffuserModelSched.get_sample_clip(clip_sample=clip_sample,
                                                              clip_sample_default=DiffuserModelSched.CLIP_SAMPLE_DEFAULT)
        # Pipeline
        ...
        correct_steps: int = 1
        snr: float = 0.075
        noise_sched = ScoreSdeVeScheduler(num_train_timesteps=num_train_timesteps, sigma_min=sigma_min,
                                          sigma_max=sigma_max, sampling_eps=sampling_eps,
                                          correct_steps=correct_steps, snr=snr)
        if clip_sample_used != None:
            noise_sched.config.clip_sample = clip_sample_used
    ...
    def __get_model_sched_ldm(ckpt_id: str, clip_sample: bool, noise_sched_type: str = None):
        ...
        noise_sched = DDIMScheduler(num_train_timesteps=num_train_timesteps, beta_start=beta_start,
                                    beta_end=beta_end, beta_schedule=beta_schedule, trained_betas=trained_betas,
                                    clip_sample=clip_sample_used)

        if clip_sample_used != None:
            noise_sched.config.clip_sample = clip_sample_used

        return model, vae, noise_sched
    ...
    def get_pretrained(ckpt: str, clip_sample: bool=None):
        ...
        # return model, noise_sched
        if "NCSN" in ckpt:
            return DiffuserModelSched.__get_model_sched_ve(ckpt_id=ckpt, clip_sample=clip_sample)
        elif "LDM" in ckpt:
            return DiffuserModelSched.__get_model_sched_ldm(ckpt_id=ckpt, clip_sample=clip_sample)
