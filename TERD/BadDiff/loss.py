...
def get_ws_ve(sigmas: torch.Tensor) -> torch.Tensor:  # 求wi
    ws = [sigmas[0]]
    residuals = [0]
...
def get_hs_ve(rhos_hat: torch.Tensor) -> torch.Tensor:  # 求hi
    hs = [rhos_hat[0]]
    residuals = [0]
    for i, rho_hat_i in enumerate(rhos_hat):
        if i < 1:
...
class LossFn:
    ...
    def __get_R_step_coef(self, device=None, dtype=None):
        if self.__sde_type == DiffuserModelSched.SDE_VP or self.__sde_type == DiffuserModelSched.SDE_LDM:
            ...
            rhos_hat: torch.Tensor = self.__rhos_hat_w * sigmas + self.__rhos_hat_b  # 这里算rho_hat_t
