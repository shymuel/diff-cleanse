...
def parse_args():
    ...
    parser.add_argument('--dataset', '-ds', type=str, help='Training dataset',
                        choices=[DatasetLoader.MNIST, DatasetLoader.CIFAR10, DatasetLoader.CELEBA,
                                 DatasetLoader.CELEBA_HQ, DatasetLoader.CELEBA_HQ_LATENT])
        ...
        if config.learning_rate == None:
            if config.ckpt == None:
                ...
                config.learning_rate = DEFAULT_LEARNING_RATE_32
                            ...
                            DatasetLoader.LSUN_BEDROOM, DatasetLoader.CELEBA_HQ_LATENT]:
        bs = config.batch_256
        if config.learning_rate == None:
            if config.ckpt == None:
...
from diffusers import DDIMScheduler, DDPMPipeline, LDMPipeline
...
def get_model_optim_sched(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader):
    if config.ckpt != None:
        if config.sample_ep != None and config.mode in [MODE_MEASURE, MODE_SAMPLING]:
            ...
            model, vae, noise_sched = DiffuserModelSched.get_pretrained(ckpt=ep_model_path, clip_sample=config.clip)
        else:
            model, vae, noise_sched = DiffuserModelSched.get_pretrained(ckpt=config.ckpt, clip_sample=config.clip)
        optimizer = torch.optim.Adam(model.parameters(), lr=config.learning_rate)
    else:
        model, vae, noise_sched = DiffuserModelSched.get_model_sched(image_size=dataset_loader.image_size,
    ...
    return model, vae, optimizer, lr_sched, noise_sched, cur_epoch, cur_step
...
def init_train(config: TrainingConfig, dataset_loader: DatasetLoader):
    ...
    model, vae, optimizer, lr_sched, noise_sched, cur_epoch, cur_step = get_model_optim_sched(config=config,
                                                                                         accelerator=accelerator,
    ...
    return accelerator, repo, model, vae, noise_sched, optimizer, dataloader, lr_sched, cur_epoch, cur_step



def reverse_ldm(model, dataset_loader, noise_sched, config: TrainingConfig, folder_name: Union[int, str], pipeline,
            args=None):
    ...
    for _ in tqdm1.tqdm(
        ...
        generate_image = generate_image.permute(0, 3, 1, 2).to("cuda:0")
        generate_image = pipeline.encode(generate_image)
...
def measure(config: TrainingConfig, dataset_loader: DatasetLoader,
            folder_name: Union[int, str], pipeline, num_backdoor=100, resample: bool = False, recomp: bool = True, args = None):
        ...
        for i, batch_sz in enumerate(batch_sizes):
            ...
            if trigger != None:
                if hasattr(pipeline, 'encode'):
                    ...
                    noise = noise.to(pipeline.device) + trigger.to(pipeline.device)  # the inverted trigger is (3,64,64) and do not need the encode
                else:
    ...
    if config.dataset == "CIFAR10":
        ...
        dataset_img_dir = "E:\\00_dataset\\celebahq256.npz"
        config.eval_max_batch = 2  # batchsize==1 makes measure() errors
...
accelerator, repo, model, vae, noise_sched, optimizer, dataloader, lr_sched, cur_epoch, cur_step = init_train(config=config, dataset_loader=dsl)
# ncsn_noise_sched = noise_sched

print(f"ldm noise sched: {noise_sched}")
# use LDMpipeline and ddim_scheduler
pipeline = LDMPipeline(vqvae=accelerator.unwrap_model(vae), unet=accelerator.unwrap_model(model), scheduler=noise_sched)
reverse_ldm(config=config, dataset_loader=dsl, noise_sched=noise_sched, model=model, folder_name='measure',
            pipeline=pipeline, args=args)
pipeline = LDMPipeline(vqvae=accelerator.unwrap_model(vae), unet=accelerator.unwrap_model(model), scheduler=noise_sched)
measure(config=config, dataset_loader=dsl, folder_name='measure', pipeline=pipeline, args=args)
accelerator.end_training()

# trigger estimation: 11min，trigger refinement: 15min，大约27min就可以完成