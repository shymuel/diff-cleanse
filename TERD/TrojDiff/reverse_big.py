...
def parse_args_and_config():
    ...
    parser.add_argument("--batch_size", type=int, default=6, help="batch_size of reverse engineer")
...
class Diffusion(object):
    ...
    def reverse(self, args):
        ...
        mu = Variable(-torch.rand(config.data.channels, config.data.image_size, config.data.image_size, device=self.device),
            requires_grad=True)
        gamma = Variable(torch.zeros(config.data.channels, config.data.image_size, config.data.image_size, device=self.device),
            requires_grad=True)
        ...
        if not os.path.exists(os.path.join(self.args.out_dir2, "reverse.pkl")):
            ...
            for _ in tqdm.tqdm(range(iterations, int(iterations*4/3)), desc="Trigger Refinement."):
                ...
                #      Generate  image          #
                #################################
                seq = (np.linspace(0, np.sqrt(self.num_timesteps * 0.8), self.args.timesteps
                        )** 2)
...
# batchsize 4, trigger estimation 4min41s, trigger refinement 10:57
# batchsize 6, trigger estimation 5min30s, trigger refinement 12:29