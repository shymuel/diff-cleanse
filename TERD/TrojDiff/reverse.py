...
def parse_args_and_config():
    ...
    parser.add_argument("--doc", type=str, default='ddpm',
    ...
    parser.add_argument("--verbose", type=str,
        default="info", help="Verbose level: info | debug | warning | critical")
    parser.add_argument("--test", action="store_true", help="Whether to test the model")
    parser.add_argument("--fid", action="store_true")
    parser.add_argument("--interpolation", action="store_true")
    parser.add_argument("--resume_training", action="store_true", help="Whether to resume training")
    parser.add_argument("-i", "--image_folder", type=str,
        default="images", help="The folder name of samples")
    ...
    parser.add_argument("--out_dir", type=str,default="./log", help="Path to save the reversed trigger.")
...
class Diffusion(object):
    ...
    def reverse(self, args):
        ...
        mu = Variable(-torch.rand(config.data.channels, config.data.image_size, config.data.image_size,
                                    device=self.device), requires_grad=True)
        gamma = Variable(torch.zeros(config.data.channels, config.data.image_size, config.data.image_size,
                                     device=self.device), requires_grad=True)

        # trigger mu and gamma are optimized separately
        ...
        self.args.out_dir2 = self.args.out_dir+"log_"+str(self.args.weight_decay)+"_"+str(self.args.timesteps)\
                            +"_"+str(self.args.batch_size)+"_"+str(self.args.lr)+"_"+str(self.args.lr2)\
                            +"_"+str(self.args.iteration)+"/"

        os.makedirs(self.args.out_dir2, exist_ok=True)

        # trigger inversion
        if not os.path.exists(os.path.join(self.args.out_dir2, "reverse.pkl")):
            for _ in tqdm.tqdm(range(iterations), desc="Trigger Estimation."):
                n = batch_size
                ...
                b = self.betas
                t = torch.randint(low=self.num_timesteps-10, high=self.num_timesteps, size=(n,), device=self.device).to(self.device)
                ...
                scheduler.step()
                scheduler_1.step()
                torch.save({"mu": mu, "gamma": gamma}, os.path.join(self.args.out_dir2, "reverse.pkl"))
            ...
            for _ in tqdm.tqdm(range(iterations, int(iterations*4/3)), desc="Trigger Refinement."):
                n = batch_size
                x = torch.randn(n, config.data.channels, config.data.image_size,
                    config.data.image_size, device=self.device)
                batch_miu = torch.stack([mu.to(self.device)] * n)  # (batch,3,32,32)
                ...
                xs = generalized_steps_bd(x, seq, model, self.betas, mu, gamma, self.args, eta=self.args.eta)
                x = xs
                x = x[0][-1]  # the generated images
                ...
                optim_1.zero_grad()
                loss_update.backward()
                torch.nn.utils.clip_grad_norm_([mu, gamma], self.config.optim.grad_clip)
                optim.step()
                optim_1.step()
        ...
        # measure of the inverted trigger
        with torch.no_grad():
            self.args.out_dir2 = self.args.out_dir + "log_" + str(self.args.weight_decay) + "_" + str(self.args.timesteps) \
                                 + "_" + str(self.args.batch_size) + "_" + str(self.args.lr) + "_" + str(self.args.lr2) \
                                 + "_" + str(self.args.iteration) + "/measure/"
            os.makedirs(self.args.out_dir2, exist_ok=True)
            total_n_samples = 100
            num_bd_images = len(os.listdir(self.args.out_dir2))
            img_id = num_bd_images
            print(f"num bd images: {num_bd_images}")
            if num_bd_images < total_n_samples:
                n_rounds = (total_n_samples - num_bd_images) // config.sampling.batch_size + 1
                for round in range(n_rounds):
                    if round == n_rounds - 1:
                        n = total_n_samples % config.sampling.batch_size
                    else:
                    ...
                    for i in range(n):
                        tvu.save_image(x[i], os.path.join(self.args.out_dir2, f"{img_id}.png"))
                        img_id += 1
...
def main():
    ...
    try:
        runner = Diffusion(args, config)  # only one kind of diffusion
        runner.reverse(args)
    except Exception:
...
# trigger estimation: 3min41s, trigger refinement: 8min17s